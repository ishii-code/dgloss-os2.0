...
> **最短ルート（自分で・無料・約10分）**：まず A-1〜A-4 だけやれば、Drive/Gmailを本人権限で検索する土台が完成する（OAuthクライアントID/シークレットが手に入る）。
> **A-5（Chatアプリ＋サービスアカウント鍵）は後回しでOK** ＝ Google Chat から話しかける入口を付けるときに実施。ここだけ少し手数なので、まずは A-1〜A-4 で動かす。
> 誰かに頼む必要があるのは「組織でGCPプロジェクト作成が制限されている場合に、管理者へ作成許可を頼む」ことだけ（それ以外は全部セルフサービス）。
...
- ※ **課金・クレジットカードは不要**。本システムが使うGCPは Chat/Drive/Gmail API と OAuth のみで、いずれも無料。有料GCPサービス（Cloud Run/KMS等）は使わない（compute=Vercel・DB=Supabase）。
- ※ Google Workspace とは別物。Workspaceを使っていてもGCPプロジェクトは別に必要。
- ※ 既存のdgloss用GCPプロジェクトがあればそれでも可（APXはAWSなので該当せず・通常は新規でよい）。
- ※ 組織でGCPプロジェクト作成が制限されている場合のみ、Workspace/Cloud管理者に作成許可を依頼。
...
### A-5 Chatアプリ登録＋投稿用サービスアカウント（後回しでOK・Chatボット公開時）
